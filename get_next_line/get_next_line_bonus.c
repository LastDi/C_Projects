...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/12/07 15:50:19 by oalvera           #+#    #+#             */
/*   Updated: 2021/12/07 15:54:15 by oalvera          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0)
		return (NULL);
	save_str[fd] = ft_read(fd, save_str[fd]);
	if (!save_str[fd])
		return (NULL);
	line = ft_create_line(save_str[fd]);
	save_str[fd] = ft_del_line_from_save(save_str[fd]);
	return (line);
}
